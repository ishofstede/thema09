...
Considering our dataset is highly imbalanced, we can already guess that trees and boost algorithms will perform better than any other algorithms. Regardless, the selection of machine learning models has been made to cover to include representatives of all classifier categories:
...
Some of these algorithms, like Naive Bayes,  will not work well with imbalanced datasets. We can fix this by either under- or oversampling our dataset. This sampling will be done in Wekka for ease of use. 

parameters: 
number of repetitions = 10 
Using the default settings of Wekka, all algoritms scored higher than the ZeroR algorithm. 

Hyperparameters; 

*verschillende dafasets
-met/zonder NAs
-met/zonder logtransformatie
-met/zonder standardisatie/normalisatie
-verschillende selectie van attributen (welke variabelen zijn het beste?)
in wekka de comparisons met experimenter -> laden in R 
runtimes/ttests/barplot van welk algo het beste doet

Er is een experiment in uitgevoerd in Wekka met de default waarden van alle boven vernoemde algoritmes. 
```{r}
...
De resultaten wijzen zoals voorspeld op de tree en nearest neighbor classification aloritmen. 
