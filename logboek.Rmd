...
results_default <- aggregate(weka_default$Percent_correct, list(weka_default$Key_Scheme), mean)
...
results_default
```
All the chosen algorithms scored higher than ZeroR and OneR. The most promising results come from the K-Nearest Neighbor (89.16% correct), Simple logistic (85.24% correct) and the Random Forest (84.54% correct) algorithm. The hyper parameters shall be adjusted to see if the accuracy can be heightened. 

KNN scoring above RandomForest is suspicious, especially since the data is highly correlated. This means KNN might be overfitted on default values, so we shall adjust that by using k = log(number of samples) meaning log(4620) = 3.66 -> k = 4 but because KNN is inherently overfitted we can use a slightly lower threshold, we choose k = 3. As for SimpleLogistic, the optimal number of LogitBoost iterations to perform is cross-validated, which leads to automatic attribute selection. Because of this we shall not adjust the parameters for LogitBoost All other algorithms below 80% will be removed. 
...
```{r}
tabl_wekka_exp_adjusted <- "
Dataset                   (1) function | (2) lazy. (3) trees
------------------------------------------------------------
bird_clean               (1000   85.24 |   80.97     84.47  
------------------------------------------------------------
                               (v/ /*) |   (0/1/0)   (0/1/0)
"
cat(tabl_wekka_exp_adjusted)
```

Using the experimenter we determined that increasing neighbors in KNN decreased accuracy, an indication of overfitting. RandomForest did not seem to increase or decrease in value from increasing the depth of the tree. SimpleLogistic Has not increased or decreased either  seems to be the perfect best fitting model for our dataset with nearly 85% accuracy. 
...
ggplot(SO_curve, aes(x=`False Positive Rate`, y=`True Positive Rate`, colour = Threshold)) + geom_point() + labs(title= "ROC curve of SO")
ggplot(P_curve, aes(x=`False Positive Rate`, y=`True Positive Rate`, colour = Threshold)) + geom_point() + labs(title= "ROC curve of P")
ggplot(W_curve, aes(x=`False Positive Rate`, y=`True Positive Rate`, colour = Threshold)) + geom_point() + labs(title= "ROC curve of W")
ggplot(R_curve, aes(x=`False Positive Rate`, y=`True Positive Rate`, colour = Threshold)) + geom_point() + labs(title= "ROC curve of R")
ggplot(SW_curve, aes(x=`False Positive Rate`, y=`True Positive Rate`, colour = Threshold)) + geom_point() + labs(title= "ROC curve of SW")
ggplot(T_curve, aes(x=`False Positive Rate`, y=`True Positive Rate`, colour = Threshold)) + geom_point() + labs(title= "ROC curve of T")
```
!!!!!!!!1interpretatie 
Next we shall determine the most significant attribute to be used with our model. 
...
Used Settings: 
1. lazy.IBk default 
2. trees.RandomForest default 
3. Simple Logistic default 

```{r}
tabl_algo_comparison <- "
...
--------------------------------------------------
                               (v/ /*) |   (0/3/4)
"
cat(tabl_algo_comparison)
```
The accuracy decreases the more attributes are removed, therefore no attributes shall be removed.

## Final model 
The final model shall be a default Simple Logistic algorithm, with no parameters changed and no attributes removed from the dataset. The confusion matrix is shown below;
