...
```{r}
#correlation plot (start from 2 because id has not been removed yet)
...
# Week 3
## Cleaning dataset
The 7 missing values shall be removed and a new CSV file will be created. This will be the clean dataset with which we will perform machine learning. 
```{r}
#remove the first row ID's because they are not required
...
#aggregate(df[, 1:10], list(df$type), mean,  na.rm=TRUE)
```
