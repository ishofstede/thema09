...
Adjustments KNN:
- set KNN to 3 instead of 1.
Adjustments SMO:
- linear regression was used as calibrator. 
Adjustments RandomForest:
- set depth to 10.

The results are as follows:
```{r}
#weka_adjust <- read.csv("weka_exp_adjusted.arff", header=T, comment.char = "@")
#colnames()
#results_adjust <- aggregate(weka_adjust$Percent_correct, list(weka_adjust$Key_Scheme), mean)
...
------------------------------------------------------------
bird_clean               (1000   84.98 |   80.97     84.47  
------------------------------------------------------------
                               (v/ /*) |   (0/1/0)   (0/1/0)
...
Using the experimenter we determined that KNN without overfitting decreased in accuracy. Any parameters that are adjusted in the RandomForest. LogitBoost seems to be the perfect best fitting model for our dataset. Next we shall determine the most significant attribute to be used with our model. 

## ROC curve 
To find the ROC curve we load our dataset into weka and go to the explorer, there we can go into the classify menu and use Logistic classifier to visualize the threshold curve. We do this for every attribute. Pictured below are the values for the KNN algorithm:
...
ggplot(SO_curve, aes(x=`False Positives`, y=`True Positives`)) + geom_line()
ggplot(P_curve, aes(x=`False Positives`, y=`True Positives`)) + geom_line()
ggplot(W_curve, aes(x=`False Positives`, y=`True Positives`)) + geom_line()
ggplot(R_curve, aes(x=`False Positives`, y=`True Positives`)) + geom_line()
ggplot(SW_curve, aes(x=`False Positives`, y=`True Positives`)) + geom_line()
ggplot(T_curve, aes(x=`False Positives`, y=`True Positives`)) + geom_line()
```
